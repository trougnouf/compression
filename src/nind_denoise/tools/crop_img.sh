...
# file doesn't always give out the resolution. use imagemagick's identify instead,
# and do it in many lines to make old bash versions content
oIFS="$IFS"
IFS='x'
RES=($(identify "${FP}" | grep -o -E '[0-9]{3,}x[0-9]{3,}' | tail -1))
IFS="$oIFS"
unset oIFS
