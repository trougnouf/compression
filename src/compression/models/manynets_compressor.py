...
class Balle2017ManyPriors_ImageCompressor(
    #  TODO use abstract_model.AbstractImageCompressor instead and complete it as needed
    abstract_model.AbstractHyperpriorImageCompressor
):
    def __init__(
        self,
        out_channel_N=192,
        ...
        **kwargs,
        ...
        if model_param == "2018" or model_param is None or model_param == "None":
            ...
            self.Decoder = Balle2018PT_compressor.Synthesis_net(
                out_channel_N=out_channel_N,
                out_channel_M=out_channel_M,
                activation_function=activation_function,
            )
        elif model_param == "2017":
            self.Encoder = Balle2018PT_compressor.Analysis2017_net(
                out_channel_N=out_channel_N, out_channel_M=out_channel_M
            )
            self.Decoder = Balle2018PT_compressor.Synthesis2017_net(
                out_channel_N=out_channel_N, out_channel_M=out_channel_M
            )
        else:
    ...
    def update_entropy_table(self):
        try:
            ...
            targets = torch.linspace(
                self.min_feat - 0.5, self.max_feat + 0.5, ntargets
            ).to(self.device)
            targets = (
                targets.expand(1, self.out_channel_M, ntargets)
                .unsqueeze(-1)
                .contiguous()
            )  # contiguous?
            entropy_table = self.bitEstimators(targets).squeeze(-1).squeeze(1)
            self.entropy_table_float = entropy_table.squeeze()
            self.entropy_table = torchac._renorm_cast_cdf_(
                entropy_table.unsqueeze(0), self.precision
            ).squeeze(0)
            self.entropy_table_cpu = self.entropy_table.cpu()

            for dist_i in range(self.num_distributions):
                assert torch.equal(
                    self.entropy_table[dist_i],
                    torchac._renorm_cast_cdf_(
                        entropy_table[dist_i].unsqueeze(0).unsqueeze(0), self.precision
                    )
                    .squeeze(0)
                    .squeeze(0),
                )
    ...
    def encode(self, input_image, entropy_coding=True, out_fpath=None):
        ...
        feat2enc = (
            feature.view(nch, -1).transpose(0, 1).unsqueeze(-1)
        )  # .contiguous().long()
        min_feat = feat2enc.min()
        max_feat = feat2enc.max()
        if min_feat < self.min_feat or max_feat > self.max_feat:
            print(
                f"encode: warning: min_feat={min_feat}, max_feat={max_feat} exceed the current CDF tables' range [{self.min_feat},{self.max_feat}]"
            )
            print(
                f"Doubling the CDF table's size in this model's instance. This will increase runtime significantly; you likely want to clip the features instead."
            )
            self.min_feat = self.min_feat - self.max_feat
            self.max_feat *= 2
            print(
                f"New values: model.min_feat={self.min_feat}, model.max_feat={self.max_feat}"
            )
        ...
        feat_indices_lower = (
            (feat2enc - min_feat)
            .unsqueeze(0)
            .expand(self.num_distributions, -1, -1, -1)
            .long()
        )
        ...
        cdf_tbl = entropy_table.unsqueeze(1).expand(
            self.num_distributions, h * w, nch, entropy_table.size(-1)
        )
        probs = torch.gather(cdf_tbl, 3, feat_indices_upper) - torch.gather(
            cdf_tbl, 3, feat_indices_lower
        )

        _, indices = torch.sum(
            torch.clamp(-1.0 * torch.log(probs + 1e-10) / math.log(2.0), 0, 50), dim=(2)
        ).min(0)

        # replace min_feat by self.mean_feat when using full range of probabilities
        cdf_tbl = (
            torch.gather(
                self.entropy_table.unsqueeze(0).expand(
                    h * w, self.num_distributions, nch, nL
                ),
                1,
                indices.view(-1, 1, 1, 1).expand(h * w, 1, nch, nL),
            )
            .squeeze(1)
            .unsqueeze(0)
        )
        ...
        features_int = (
            ((feature.view(1, nch, -1).transpose(1, 2) - self.min_feat) / self.q_intv)
            .round()
            .short()
            .flatten()
        )
        ...
        if out_fpath is not None and entropy_coding:
            ...
            #    fp.write(str(w))
            utilities.compress_png(
                indices.view(1, h, w).numpy(), out_fpath + ".png"
            )  # alternatively use compress_lzma
    ...
    def decode(self, bitstream, in_fpath=None):
        ...
        if in_fpath is not None:
            ...
            indices = torch.tensor(
                np.vstack(
                    map(np.uint16, png.Reader(in_fpath + ".png").asDirect()[2])
                ).astype(np.long)
            )
            h, w = indices.shape
        else:
        ...
        cdf_tbl = (
            torch.gather(
                self.entropy_table_cpu.unsqueeze(0).expand(
                    h * w, self.num_distributions, nch, nL
                ),
                1,
                indices.view(-1, 1, 1, 1).expand(h * w, 1, nch, nL),
    ...
    def segment(self, input_image, output_fpath):
        ...
        with torch.no_grad():
            ...
            probs = self.bitEstimators(
                compressed_feature_renorm + 0.5
            ) - self.bitEstimators(compressed_feature_renorm - 0.5)
            dist_select = torch.sum(
                torch.clamp(-torch.log2(probs + 1e-10), 0, 50), dim=(2)
            ).argmin(0)
        del compressed_feature_renorm
        del probs

        color_palette = torch.tensor(
            distinct_colors.get_color_palette(self.num_distributions), dtype=torch.int16
        )
        r = (
            color_palette[:, 0]
            .view(-1, 1, 1)
            .expand(self.num_distributions, dist_select.size(1), dist_select.size(2))
        )
        g = (
            color_palette[:, 1]
            .view(-1, 1, 1)
    ...
    def timing_analysis(self, input_image, entropy_coding=False):
        ...
        try:
            ...
            logger.info(
                "timing_analysis: whole encode timing not completed due to missing torchac library"
            )
        for rep in range(50):
            with torch.no_grad():
                # one whole enc
                ...
                try:
                    ...
                    features_int = (
                        (
                            (feature.view(1, nch, -1).transpose(1, 2) - self.min_feat)
                            / self.q_intv
                        )
                        .round()
                        .short()
    ...
    def forward(self, input_image, **kwargs):
        ...
        Return (reconstructed_image, visual_loss, bpp_main, bpp_sidestring, used_distributions,
        ...
        if self.dist_patch_size == 1 and self.nchans_per_prior == self.out_channel_M:
            ...
            im_batched = pt_ops.img_to_batch(
                compressed_feature_entropy, self.dist_patch_size, self.nchans_per_prior
            )
            feature_batched_shape = im_batched.shape
            probs = self.bitEstimators(im_batched + 0.5) - self.bitEstimators(
                im_batched - 0.5
            )
            total_bits = torch.sum(
                torch.clamp(-torch.log2(probs + 1e-10), 0, 50), dim=(2, 3, 4)
            )  # per dist_i, batch
            minbits, dist_select = total_bits.min(0)
        ...
        if self.training:
            bpp_sidestring = torch.tensor(
                ...
                / (im_shape[0] * im_shape[2] * im_shape[3])
            )
        else:
            bpp_sidestring = (
                torch.tensor(
        ...
        #     bpp_sidestring,
        #     bpp,
        #     used_dists_cpu.tolist(),
        #     num_dists_to_force_train,
        # )
    ...
    def entropy_encode_find_best_priors(self, features, *args):
        ...
        for bn in range(bs):
            features_int = (
                features[bn].reshape(1, nch, -1).transpose(1, 2)
            )  # bs, h*w, nch
            features_int = (
                ((features_int - self.min_feat) / self.q_intv).round().short()
            )
            for adim in range(h * w):
                ...
                for adist in range(1, self.num_distributions):
                    cdf_tbl = (
                        self.entropy_table[adist].unsqueeze(0).unsqueeze(0)
                    )  # 1, 1, nch, L
    ...
    def entropy_encode_use_estim_prior(self, features, dist_select):
        ...
        for bn in range(bs):
            ...
            #     )
            features_int = (
                features[bn].reshape(1, nch, -1).transpose(1, 2).contiguous()
            )  # bs, h*w, nch
            features_int = (
                ((features_int - self.min_feat) / self.q_intv).round().short().flatten()
            )
            ...
            # print(decoded_str)
            decoded_str = (
                decoded_str.view(1, h * w, nch).transpose(1, 2).reshape(1, nch, h, w)
                * self.q_intv
                + self.min_feat
            ).float()
            if features.max() > self.max_feat or features.min() < self.min_feat:
                logger.info(
                    "entropy_encode error: z out of range: [{}, {}] instead of [{}, {}]".format(
                        features.min(), features.max(), self.min_feat, self.max_feat
                    )
                )
                breakpoint()
