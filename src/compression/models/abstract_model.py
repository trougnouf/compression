"""Base model for hyperprior based network."""
# -*- coding: utf-8 -*-
...
class AbstractHyperpriorImageCompressor(AbstractImageCompressor):
    """
    Parameters:
        lossf: can be a string (mse, ssim, msssim), function, or None
    """
    ...
    def __init__(
        ...
        min_feat=-127,
        max_feat=128,
        min_feat_gaussian=-127,
        max_feat_gaussian=128,
        q_intv=1,
        precision=16,
        entropy_coding=False,
        conditional_distribution="Laplace",
        passthrough_ae=False,
        **kwargs,
    ):  # , out_channel_M=320, lossf='mse'):
        ...
        self.min_feat, self.max_feat, self.q_intv, self.precision = (
            min_feat,
            max_feat,
            q_intv,
            precision,
        )
        self.min_feat_gaussian, self.max_feat_gaussian = (
            min_feat_gaussian,
            max_feat_gaussian,
        )
        ...
        if conditional_distribution == "Laplace":
            ...
            or conditional_distribution == "Normal"
        ):
    ...
    def forward(self, input_image, **kwargs):
        ...
        total_bits_feature, _ = feature_probs_based_sigma(
            compressed_feature_entropy, recon_sigma
        )
        # except ValueError as e:
        #    print(f"abstract_model error {e}")
        #    breakpoint()
    ...
    def update_entropy_table(self):
        ...
        ntargets = int(
            (-self.min_feat + self.max_feat) / self.q_intv + 2
        )  # +2 = 19000->6000 DBG
        targets = torch.linspace(self.min_feat - 0.5, self.max_feat + 0.5, ntargets)
        targets = (
            targets.expand(self.out_channel_N, ntargets).unsqueeze(0).unsqueeze(-1)
        )  # 1, nch, L, 1
        probs = self.bitEstimator_z(targets.to(self.device))

        self.z_entropy_table = (
            _renorm_cast_cdf_((probs).squeeze(-1).unsqueeze(0), self.precision)  # DBG
            .squeeze(0)
            .squeeze(0)
        )  # 1,1,ch,L -> ch,L
        ...
        ntargets = int(
            (-self.min_feat_gaussian + self.max_feat_gaussian) / self.q_intv + 2
        )  # +2 = 19000->6000 DBG
        targets = torch.linspace(
            self.min_feat_gaussian - 0.5, self.max_feat_gaussian + 0.5, ntargets
        )
        # #breakpoint()
        self.main_targets = targets

        self.scale_table = np.exp(
            np.linspace(np.log(0.11), np.log(256), NUMSCALES)
        )  # 256
        self.scale_table = np.exp(
            np.linspace(np.log(0.0001), np.log(128), NUMSCALES)
        )  # 256
        ...
        for scalenum, ascale in enumerate(self.scale_table):
            self.conditional_CDF_float[scalenum] = self.conditional_distribution(
                0, ascale
            ).cdf(self.main_targets)
            self.conditional_CDF[scalenum] = (
                _renorm_cast_cdf_(
                    self.conditional_CDF_float[scalenum].unsqueeze(0), self.precision
                )
                .squeeze(0)
                .squeeze(0)
            )
    ...
    def entropy_encode(self, z):
        ...
        for bn in range(bs):
            ...
            if not torch.equal(z, decoded_str.float()):
                ...
                logger.info(
                    "entropy_encode error: z out of range: [] instead of []"
                ).format(z.min(), z.max(), self.min_feat, self.max_feat)
                breakpoint()
    ...
    def encode(self, input_image, entropy_coding=True):
        ...
        feature_renorm = (
            ((feature_renorm.flatten() - self.min_feat_gaussian) / self.q_intv)
            .round()
            .short()
        )
        ...
        cdf_z = (
            self.z_entropy_table.unsqueeze(0)
            .expand(z_h * z_w, z_nch, z_nL)
            .unsqueeze(0)
        )
        z_int = (
            ((z.view(1, z_nch, -1).transpose(1, 2) - self.min_feat) / self.q_intv)
            .round()
            .short()
            .flatten()
        )
        ...
        if GAUSSIAN_DISTRIBUTION_USES_LUT:
            ...
            cdf_main = (
                gaussian.cdf(self.main_targets.unsqueeze(-1).expand(-1, sigma.size(0)))
                .transpose(0, 1)
                .unsqueeze(0)
                .unsqueeze(0)
            )
    ...
    def timing_analysis(self, input_image, entropy_coding=False):
        ...
        for rep in range(50):
            with torch.no_grad():
                ...
                z_int = (
                    (
                        (z.view(1, z_nch, -1).transpose(1, 2) - self.min_feat)
                        / self.q_intv
                    )
                    .round()
                    .short()
                ...
                encoded_zstr = torchac.encode_cdf(
                    cdf_z.contiguous(), z_int.contiguous()
                )  # comment to rm entropy coding
                ...
                if GAUSSIAN_DISTRIBUTION_USES_LUT:
                    ...
                    gaussian = self.conditional_distribution(
                        mu.flatten(), sigma.flatten()
                    )
                    cdf_main = (
                        gaussian.cdf(
                            self.main_targets.unsqueeze(-1).expand(-1, sigma.size(0))
                        )
                        .transpose(0, 1)
                        .unsqueeze(0)
                        .unsqueeze(0)
                ...
                cdf_indices = (
                    (
                        recon_sigma
                        - torch.tensor(self.scale_table)
                        .view(NUMSCALES, 1, 1, 1)
                        .expand(NUMSCALES, feat_nch, feat_h, feat_w)
                    )
                    .abs()
                    .argmin(0)
                )
                ...
                cdf_indices = cdf_indices.view(1, feat_nch, -1, 1).expand(
                    -1, -1, -1, z_nL
                )
                ...
                exp_cdf_flt = self.conditional_CDF_float.view(
                    -1, 1, 1, z_nL
                )  # 64,1,1,257)
                exp_cdf_flt = exp_cdf_flt.expand(-1, feat_nch, feat_h * feat_w, z_nL)
                cdf_tbl_main_flt = torch.gather(exp_cdf_flt, 0, cdf_indices)
                feat_indices_lower = (
                    feature_renorm.view(1, feat_nch, -1, 1)
                ).long() - self.min_feat
                feat_indices_upper = feat_indices_lower + 1
                probs_flt = (
                    torch.gather(cdf_tbl_main_flt, 3, feat_indices_upper)
                    - torch.gather(cdf_tbl_main_flt, 3, feat_indices_lower)
                ).float()
                ...
                total_bits = torch.sum(
                    torch.clamp(
                        -1.0 * torch.log(probs_flt + 1e-10) / math.log(2.0), 0, 50
                    )
                )
                logger.info("quant: ")
                ...
                gaussian = self.conditional_distribution(
                    mu.view(sigma.shape).contiguous(), sigma
                )
                probs = gaussian.cdf(feature + 0.5) - gaussian.cdf(feature - 0.5)
                total_bits = torch.sum(
                    torch.clamp(-1.0 * torch.log(probs + 1e-10) / math.log(2.0), 0, 50)
    ...
    def entropy_encode_per_channel(self, z):
        ...
        for bn in range(bs):
            for ch in range(nch):
                cdf_tbl = (
                    self.z_entropy_table[ch]
                    .expand(1, h, w, self.z_entropy_table.size(-1))
                    .contiguous()
                )
                z_int = (
                    ((z[bn, ch] - self.min_feat) / self.q_intv)
                    .round()
                    .short()
                    .flatten()
                )
    ...
    def get_parameters(self, lr=None):
        ...
        param_list = [
            ...
            {
                "params": self.bitEstimator_z.parameters(),
                "lr": lr * 10,
                "name": "bit_estimator",
            },
    ...
    def complexity_analysis(self):
        ...
        macs, params = ptflops.get_model_complexity_info(
            ...
            custom_modules_hooks={GDN.GDN: GDN.gdn_flops_counter_hook},
        )

        decoder = self.get_decoding_class()
        macs, params = ptflops.get_model_complexity_info(
            decoder,
            (LATENTCH, IMGHEIGHT // 16, IMGWIDTH // 16),
            custom_modules_hooks={GDN.GDN: GDN.gdn_flops_counter_hook},
        )
