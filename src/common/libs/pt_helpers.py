...
sys.path.append("..")
from common.libs import utilities, np_imgops
...
def tensor_to_imgfile(tensor, path):
    ...
    if tensor.dtype == torch.float32:
        if path[-4:].lower() in [".jpg", "jpeg"]:  # 8-bit
            ...
            if (
                math.floor(tensor.max()) > 1 and tensor.max() <= 255
            ):  # 8 bit expressed as 0-255. should have been uint8 but handle it here anyway.
                print("tensor_to_imgfile: warning: float tensor interpreted as uint8")
                nptensor = (
                    tensor.round().cpu().numpy().astype(np.uint8).transpose(1, 2, 0)
                )
            else:  # 16 bit
                nptensor = (
