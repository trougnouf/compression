# -*- coding: utf-8 -*-
"""
Tools that downloads categories from Wikimedia Commons.

egrun:
    python3 tools/wikidownloader.py --category "Category:Quality images of molluscs"
(run from "common" directory)

eg sources of high quality categorized images:
    https://commons.wikimedia.org/wiki/Category:Quality_images_by_subject
    https://commons.wikimedia.org/wiki/Category:Featured_pictures_by_subject

Python dependencies: mwclient, tqdm
"""
import mwclient
import json
import os
...
DLDIR = {"FP": "FeaturedPictures", "QI": "QualityImages%u" % YEAR["QI"]}
CAT = {
    "FP": "Category:Featured_pictures_on_Wikimedia_Commons",
    "QI": "Category:Quality_images",
}
...
SITE = mwclient.Site(
    ...
    clients_useragent="TrougnoufDownloader/0.1 (b.brummer@intopix.com)",
)
...
def get_cat_imgs(rootcat=CAT["FP"], site=SITE, known_cats=set(), subcats=True) -> list:
    ...
    while cmcontinue is not None or queryres is None:
        ...
        for afile in queryres["query"]["categorymembers"]:
            res.add(afile["title"])
        cmcontinue = (
            None if "continue" not in queryres else queryres["continue"]["cmcontinue"]
        )
    ...
    for i, apic in enumerate(tqdm(todl_pictures), start=1):
        ...
        if os.path.exists(target_fpath):
            ...
            if checksum == localhash:
                ...
                print(
                    f"warning: checksums do not match with local file. Re-downloading {target_fpath}."
                )
        if year is not None:
            try:
        ...
        if localhash != checksum:
            print(
                f"warning: checksums do not match; {target_fpath} is likely corrupted. You can download it manually or start this script over."
            )
...
def parser_add_arguments(parser):
    parser.add_argument(
        "--cat",
        default="FP",
        help="Category short name (presets). default: FP, options: {}".format(
            CAT.keys()
        ),
    )
    parser.add_argument(
        "--category",
        help="Category long name (what follows Category: on Wikimedia Commons)",
    )
    parser.add_argument(
        "--dest_dirname",
        help="Destination directory name (default: Category:<CATEGORY>))",
    )
    ...
    parser.add_argument(
        "--rootdir",
        default=ROOTDIR,
        help="Root download directory (default: {})".format(ROOTDIR),
    )
    parser.add_argument(
        "--delete_uncat",
        action="store_true",
        help="Delete images which are no longer part of the category",
    )
...
if __name__ == "__main__":
    ...
    dl_cat(
        rootcat=args.category,
        limit=args.limit,
        dlrootdir=args.rootdir,
        dldir=args.dest_dirname,
        year=args.year,
        delete_uncat=args.delete_uncat,
    )
