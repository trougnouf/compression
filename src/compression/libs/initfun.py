...
# -*- coding: utf-8 -*-
"""
Initialization functions used in compression and dc.

common args, checkpoint finder, args and jsonsaver initializer
...
DEFCONF_FPATHS = [os.path.join("..", "compression", "config", "defaults.yaml")]
...
def common_parser_add_arguments(parser) -> None:
    # very useful config
    parser.add_argument(
        "--expname", help="experiment name (default is autogenerated from config file)"
    )
    parser.add_argument(
        "-p",
        "--pretrain",
        help=(
            "load pretrain model. argument can be an experiment name, an"
            "experiment directory path, or a specific model file"
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--arch", help="[*]_ImageCompressor class name as defined in models"
    )
    parser.add_argument(
        "--train_lambda", type=int, help="RD tradeoff (higher = better IQ"
    )
    parser.add_argument("--lossf", help="mse, msssim, ssim")
    parser.add_argument(
        "--num_distributions",
        type=int,
        help="Number of distributions in ManyPriors architectures",
    )
    # moderately useful config
    parser.add_argument(
        "--val_dpath",
        dest="val_dpath",
        help="the path of validation dataset (used to decay the learning rate)",
    )
    parser.add_argument(
        "--test_std_dpaths",
        nargs="+",
        help=("Path(s) to the test dataset(s)"),
    )
    ...
    parser.add_argument(
        "--no_passthrough_ae", dest="passthrough_ae", action="store_false"
    )
    parser.add_argument(
        "--stage_switches",
        type=float,
        nargs="*",
        help="Ratio of steps after which model switches to next state",
    )
    # very unusual config
    ###############################################################################################
    parser.add_argument(
    ...
    parser.add_argument(
        "--global_step", type=int, help="Starting step (does not influence pretrain)"
    )
    parser.add_argument(
        "--entropy_coding",
        action="store_true",
        help="Perform entropy coding to determine actual bpp of bitstream",
    )
    parser.add_argument("--out_channel_H", help="Model parameter")
    parser.add_argument(
        "--synthesis_prior_arch",
        help="Class name as defined in models (for Balle2018 scheme override)",
    )
    parser.add_argument(
        "--analysis_arch",
        help="Class name as defined in models (for Balle2018 scheme override)",
    ...
    parser.add_argument(
        "--consistent_patch_size",
        action="store_true",
        help="Use the same patch size for testing as for training",
    )
    parser.add_argument("--image_size", "--cs", type=int, help="Crop size")
    parser.add_argument(
        "--dist_patch_size",
        type=int,
        help="Number of features represented by the distribution (deprecated)",
    )
    parser.add_argument(
        "--nchans_per_prior",
        help="Number of channels per prior distribution (blank=all)",
    )
    parser.add_argument("--model_param", help="A class specific parameter")
    parser.add_argument(
        "--conditional_distribution",
        help="Conditional distribution used in Balle2018 type model (Gaussian, Laplace)",
    )
    parser.add_argument(
        "--activation_function",
        help="Activation function used in the main encoder/decoder",
    )
    parser.add_argument(
        "--bitparm_init_range", type=float, help="Entropy model init parameter"
    )
    parser.add_argument("--bitparm_init_mode", help="Entropy model init mode")
    parser.add_argument(
        "--freeze_autoencoder",
        action="store_true",
        help="Freeze autoencoder to fine-tune entropy model",
    )
    parser.add_argument(
        "--freeze_autoencoder_steps",
        type=int,
        help="Freeze autoencoder to fine-tune entropy model",
    )
    parser.add_argument(
        "--two_worse_before_lr_update",
        action="store_true",
        help="LR update after two worse epochs instead of 1",
    )
    parser.add_argument(
        "--pretrain_prefix",
        help=(
            "use test to load the best checkpoint for testing, or val to continue training"
            " (autodetected)"
        ),
    )
    parser.add_argument(
        "--save_path",
        help="Save path, this argument is normally autogenerated as args.CHECKPOINTS_DPATH/expname",
    )
    parser.add_argument("--test_cs", type=int, help="Test crop size")
    parser.add_argument(
        "--nolog",
        action="store_true",
        help="No log to json (not implemented everywhere)",
    )
    parser.add_argument(
        "--test_crop_mult",
        type=int,
        help="Test crop size will be a multiple of this value",
    )
    parser.add_argument(
        "--checkpoints_dpath", help="Root directory where models are saved"
    )


def get_args(
    specific_parser_add_arguments_fun: Callable,
    specific_parser_autocomplete_fun: Callable,
    cli_args: Optional[List[str]] = None,
    def_config_fpaths: List[str] = DEFCONF_FPATHS,
    dump_args=True,
):
    """
    Parse arguments and return them along with the jsonsaver object, given
    add_argument and autocomplete functions.
    """
    parser = configargparse.ArgumentParser(
        description=__doc__,
        default_config_files=def_config_fpaths,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
    )
    common_parser_add_arguments(parser)
    if isinstance(specific_parser_add_arguments_fun, list):
        for afun in specific_parser_add_arguments_fun:
            afun(parser)
    else:
        specific_parser_add_arguments_fun(parser)
    args, unknown_args = parser.parse_known_args(cli_args)  # args is useful for testing
    if unknown_args:
        print(
            f"get_args WARNING: unknown arguments: {unknown_args}"
        )  # TODO use logging
    logger.info(
        "get_args: unknown arguments: {}".format(args)
    )  # FIXME this doesn't print
    # args = args[0]
...
def common_init_parser_autocomplete(args: configargparse.Namespace) -> None:
    ...
    if args.config is None and args.pretrain is not None:
        ...
        args.config = os.path.join(args.checkpoints_dpath, args.pretrain, "config.yaml")
        logger.info(
            'common_init_parser_autocomplete: missing config. Did you mean to run with "--config {}"?'.format(
                os.path.join(args.checkpoints_dpath, args.pretrain, "config.yaml")
            )
        )
    ...
    if args.expname is None:
        if args.config is None:
            ...
            # use directory where config is located as expname
            args.expname = utilities.get_leaf(args.config).split(".")[0]
        args.expname += "-L" + str(args.train_lambda)  #  add lambda
        args.expname += "-" + platform.node()  # add hostname

        logger.info(
            "common_init_parser_autocomplete: expname set to {}".format(args.expname)
        )
    ...
    if args.expname != "None":
        ...
        shutil.copyfile(
            os.path.join(args.save_path, "cmd.sh"),
            os.path.join(
                args.save_path,
                "history",
                "cmd_{}.sh".format(datetime.datetime.now().isoformat()),
            ),
        )
    if args.config is None:
        logger.info(
            "warning: args.config was not specified. Using default parameters and CLI args. Model parameters are not automatically inferred from pretrained model."
        )
...
def dump_config(args):  # unused?
    with open(
        os.path.join(args.save_path, "history", "config_{}.yaml").format(
...
def get_jsonsaver(args):
    return json_saver.JSONSaver(os.path.join(args.save_path, "trainres.json"))
...
def get_best_checkpoint(
    ...
    if step is not None and step != 0:
        model_fpath = os.path.join(
            exp_dpath, "saved_models", "iter_{}.pth".format(step)
        )
    ...
    model_fpath = os.path.join(
        exp_dpath, "saved_models", "iter_{}.pth".format(best_iter)
    )
