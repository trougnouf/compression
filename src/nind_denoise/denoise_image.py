...
CS_UTNET, UCS_UTNET = 504, 480  # cs=184, ucs=164 might work better (but slower)
...
class OneImageDS(Dataset):
    ...
    '''
    def __init__(self, cs, ucs, ol, inimg_fpath=None, inimg_np=None, whole_image=False, pad=None):
        if inimg_np is not None:
            self.inimg = inimg_np
        elif inimg_fpath is not None:
            self.inimg = np_imgops.img_path_to_np_flt(inimg_fpath)
        else:
            raise ValueError('OneImageDS: inimg_fpath or inimg_np must be provided')
...
def make_seamless_edges(tcrop, x0, y0, ucs, overlap):
    if x0 != 0:#left
        tcrop[:,:,0:overlap] = tcrop[:,:,0:overlap].div(2)
    if y0 != 0:#top
        tcrop[:,0:overlap,:] = tcrop[:,0:overlap,:].div(2)
    if x0 + ucs < fswidth and args.overlap:#right
        tcrop[:,:,-overlap:] = tcrop[:,:,-overlap:].div(2)
...
if __name__ == '__main__':
    ...
    ds = OneImageDS(inimg_fpath=args.input, cs=args.cs, ucs=args.ucs, ol=args.overlap, whole_image=args.whole_image, pad=args.pad)
    ...
    for n_count, ydat in enumerate(DLoader):
        ...
        for i in range(ybatch.size(0)):
            ...
            if args.whole_image:
                ...
                tensimg = make_seamless_edges(tensimg, absx0, absy0, ucs=args.ucs, overlap=args.overlap)
                if args.debug:
