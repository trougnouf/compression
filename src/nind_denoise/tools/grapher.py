...
# used for the paper, not so useful now.
import os
import argparse
...
RES_DPATH = os.path.join('..', '..', 'results', 'NIND')

# Params
parser = argparse.ArgumentParser(description='Grapher')
parser.add_argument('--res_dir', default=os.path.join(RES_DPATH, 'test'), type=str, help='Results directory')
#parser.add_argument('--res_bfn', default='train_result', type=str, help='Result base filename (s.t. [res_bfn]_[epoch].txt)')
parser.add_argument('--save_dir', default=os.path.join(RES_DPATH, 'graphs', 'auto'), type=str, help='Where graphs are saved')
...
for image in images:
    _, isos = sort_ISOs(list(data[components[0]]['results'][image].keys()))
    #isos = baseisos + isos
    for component in components:
        try:
...
if not args.nojson:
    with open(os.path.join(RES_DPATH, 'res.json'), 'w') as f:
        json.dump(data, f)
