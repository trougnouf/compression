# -*- coding: utf-8 -*-
"""Tool and library used to remove non-best models according to expname/trainres.json: best_step."""
...
def cleanup_checkpoints(
    expname=None, checkpoints_dir=CHECKPOINTS_DPATH, keepers_def=None
):
    ...
    for expname in models:
        ...
        keepers.add(
            "iter_"
            + str(
                find_last_epoch(os.path.join(checkpoints_dir, expname, "saved_models"))
            )
            + "."
        )
        keepers.add(
        ...
        if len(keepers) <= 3:
            print(
                "warning: cleanup_checkpoints: {} has too few keepers ({}), aborting.".format(
                    expname, keepers
                )
            )
            continue
...
def cleanup_tests(
    expname=None, tests_dir="tests", keepers_def=None, checkpoints_dir=CHECKPOINTS_DPATH
):
    """orig: tcomp/tools/cleanup_checkpoints.py"""
    if keepers_def is None:
        keepers_def = set()
    ...
    for expname in models:
        ...
        if not os.path.isdir(os.path.join(checkpoints_dir, expname, tests_dir)):
            ...
            continue
        keepers.add(
            str(find_last_epoch(os.path.join(checkpoints_dir, expname, "saved_models")))
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoints_dpath",
        default=CHECKPOINTS_DPATH,
        help=("Path of root directory where models are saved"),
    )
    parser.add_argument(
        "--expname",
        help="(optional) Name of single experiment to clean-up (in checkpoints_dpath)",
    )
...
if __name__ == "__main__":
    args = parse_args()
    cleanup_checkpoints(expname=args.expname, checkpoints_dir=args.checkpoints_dpath)
