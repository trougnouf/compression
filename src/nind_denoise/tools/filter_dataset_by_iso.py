...
class TestFilterDataset(unittest.TestCase):
    def test_ext_bl(self):
        self.assertTrue(is_ext_blacklisted('afile.gif'))
        self.assertFalse(is_ext_blacklisted('afile.jpg'))

def is_ext_blacklisted(fn: str) -> bool:
    return any(fn.lower().endswith(badext) for badext in BLACKLISTED_EXT)
...
if __name__ == '__main__':
    ...
    for fn in tqdm(os.listdir(args.data_dpath)):
        ...
        if isoval is not None and isoval <= MAXISO:
            ...
            print(f'nind_denoise/tools/filter_dataset_by_iso.py: warning: could not read ISO value of {fn}')